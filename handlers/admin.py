...
async def statistic_endtime(message: types.Message, state: FSMContext):
    ...
    users.sort(key=lambda x: x[1] if x[1] else datetime.fromtimestamp(1000000000), reverse=True)
    ...
    pretty_string = "".join(
        [f'{user[0]} - {user[1].strftime("%d-%m-%Y")if user[1] else "null"}\n' for user in users]
    )
    ...
    if len(pretty_string) > 4096:
        ...
        await message.answer(
            f"{hcode('username')} - {hbold('end date')}\n\n{pretty_string}",
            parse_mode=types.ParseMode.HTML,
        )
