...
def update_user_payment(user_id: int, days: int) -> None:
    """
    ...
    try:
        ...
        with conn.cursor() as cursor:
            cursor.execute(
                ...
                WHEN subscription_end_date IS NULL THEN %s
...
def update_given_subscription_time(user_id: int, days: int) -> None:
    ...
    try:
        ...
        with conn.cursor() as cursor:
            ...
            UPDATE users SET subscription_end_date = CASE
                WHEN subscription_end_date IS NULL THEN %s
                WHEN subscription_end_date < %s THEN %s + INTERVAL '%s days'
                ELSE subscription_end_date + INTERVAL '%s days'
            END
            WHERE user_id = %s
            """,
            (
                datetime.now().date() + timedelta(days=days),  # первое %s - дата
                datetime.now().date() + timedelta(days=days),  # второе %s - дата для проверки
                datetime.now().date(),  # третье %s - начальная дата
                days,  # %s для INTERVAL '%s days' - количество дней
                days,  # %s для INTERVAL '%s days' - количество дней
                user_id,  # последний %s - user_id
            ),
            )
            conn.commit()
    ...
    try:
        ...
        with conn.cursor() as cursor:
            cursor.execute(
                ...
                """,
                (datetime.now().date() + timedelta(days=days), user_id),
            )
